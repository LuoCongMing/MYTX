...
#pragma mark - INputViewSetting
-(void)inputPickerViewSetting{
    
    _bgView.frame = CGRectMake(0, 0, KScreenWidth, 280*HScale);
    self.frame = CGRectMake(0, 0, KScreenWidth, 280*HScale);
}
- (void)initUI{
    ...
    _bgView = [[UIView alloc] initWithFrame:CGRectMake(0, iPhone_Height, KScreenWidth, 280*HScale)];
...
- (void)setType:(PickerViewType)type{
    ...
    switch (type) {
        ...
        {
            self.titleLab.text = @"选择你的生日";
            
           [self isDataPicker:YES];
            self.datePicke.datePickerMode = UIDatePickerModeDate;
            [self.datePicke setMinimumDate:[PickerView distanceYear:-70]];
        ...
        case PickerViewTypeCity:{
            ...
            break;
        case PickerViewTypeXingJiaoYou:{
            self.titleLab.text = @"交友";
            [self.array addObject:@[@"勿扰",@"求撩",@"走心"]];
            [self isDataPicker:NO];
        }
            break;
        case PickerViewTypeXingQuXiang:{
            self.titleLab.text = @"性取向";
            [self.array addObject:@[@"男",@"女",@"双性恋"]];
            [self isDataPicker:NO];
        }
            break;
        case PickerViewTypeXingHunYin:{
            self.titleLab.text = @"婚恋";
            [self.array addObject:@[@"单身",@"恋情",@"已婚"]];
            [self isDataPicker:NO];
        }
            break;
        default:
...
- (void)showAnimation{
    ...
    [UIView animateWithDuration:0.5 animations:^{
        ...
        frame.origin.y = iPhone_Height-280*HScale;
...
- (void)hideAnimation{
    ...
    if (self.frame.size.height == 280*HScale) {
